...
Device::Device(uint32_t id, cl::Device cl_device, cl::Context cl_context)
    : _cl_context(std::move(cl_context)), _cl_device(std::move(cl_device)), _id(id) {
...
const cl::Context& Device::get_cl_context() const { return _cl_context; }
cl::Context& Device::get_cl_context() { return _cl_context; }
...
uint64_t Device::_compute_cores() const {
...
Device DeviceManager::get<Filter::MAX_FLOPS>() {
  ...
  return *std::max_element(dm._devices.begin(), dm._devices.end(),
                           [](const Device& a, const Device& b) { return a.estimated_flops() < b.estimated_flops(); });
}
...
DeviceManager::DeviceManager() {
  ...
  for (const auto& clp : cl_platforms) {
    ...
    for (auto& cld : cl_devices) {
      ...
      _devices.emplace_back(id++, std::move(cld), std::move(cl_context));
    }
  }
}
