...
/**
 * @brief Device represents a single OpenCL Device by wrapping cl::Device.
 *        It provides instant methods for retrieving common data.
 *        The naive cl::Device and cl::Context objects can be retrieved as well.
 */
class Device {
 ...
 public:
   ...
   */
  Device(uint32_t id, cl::Device cl_device, cl::Context cl_context);
  ~Device();
  ...
  /**
   * @brief Returns a const reference to the underlying cl::Context.
   */
  [[nodiscard]] const cl::Context& get_cl_context() const;
  /**
   * @brief Returns a reference to the underlying cl::Context.
   */
 ...
 private:
  Device() = default;
  [[nodiscard]] uint64_t _compute_cores() const;
